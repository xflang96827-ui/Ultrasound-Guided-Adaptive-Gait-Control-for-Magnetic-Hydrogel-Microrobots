...
This file uses **GitHub-compatible math blocks**.
...
Use fenced math blocks rather than bracket-style display math.

This version avoids several LaTeX commands that often fail in GitHub math rendering.
...
D_t(x,y)
=
\alpha |I_t(x,y)-I_{t-1}(x,y)|
+
\beta |I_t(x,y)-I_0(x,y)|
```
...
- `alpha` controls the weight of inter-frame motion.
- `beta` controls the weight of background subtraction.
...
\begin{cases}
x_{rand}, & d<\eta \\
x_{near}+\dfrac{x_{rand}-x_{near}}{d}\eta, & \mathrm{otherwise}
\end{cases}
```
...
=
\begin{cases}
\mathrm{True}, & \mathrm{no\ collision} \\
\mathrm{False}, & \mathrm{collision}
\end{cases}
```
...
If the rewiring condition is satisfied:
...
=
\begin{bmatrix}
\frac{\partial v_x}{\partial v_y}
&
\frac{\partial v_x}{\partial \alpha}
\\
\frac{\partial v_y}{\partial v_y}
&
\frac{\partial v_y}{\partial \alpha}
\\
0 & 0
\end{bmatrix}
=
...
- `C_flow` is the flow set.
- `F` is the continuous dynamics.
- `D_jump` is the jump set.
- `G` is the jump map.
